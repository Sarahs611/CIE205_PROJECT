...
int main()
{
    ...
    Rest.LoadInputFile("input(50).txt");//open input file
    ifstream test("input(50).txt");
    if (!test.is_open())
    {
    ...
    {
        cout << "file open successfuly" << endl;
    }
    test.close();
    Rest.SimulationT(mode);//excute simulation
    ofstream outfile("output.txt");//open output file 
    Rest.OutputFile(outfile);
    outfile.close();
}
//
//    cout << " Welcome ^_^ "<<endl;
